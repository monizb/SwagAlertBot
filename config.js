...
const firebase =
    require('firebase'); // Get Firebase Instance to Initialize the DB
const keys = require("./keys");

const config = {

  /* These are your twitter developer authentication details. Get them at
  ...
  // Appended at the end of search queries to filter out some data (We dont want
  // tweets which are retweets or replies)
  SEARCH_QUERY_FILTERS : '-filter:retweets -filter:replies',

  // Filter out tweets containing any phrases you want - just add them as
  // strings in the array
  POST_SEARCH_FILTERS : [],

  // Allow searching by location. A Valid geocode has the form "lat long
  // radius_in_miles" Example: '37.781157 -122.398720 1mi'
  SEARCH_BY_GEOCODE : '',

  // 'Specifies what type of search results you would prefer to receive. The
  // current default is “mixed.” Valid values include:' Default: 'recent'
  ...
  // When we detect another Bot ,If option below is enabled, allow us to block
  // them.
  ...
  // Array of preferred accounts. If set, it only filters tweets from these
  // accounts. Example: ['user1', 'user2']
  PREFERRED_ACCOUNTS : []
};
